---
id: company-run-sec-10y-100-company-dashboard
type: report
report_kind: company_thesis_dashboard
schema: sec-company-ledger-markdown-v1
run_id: sec-10y-100-company
entity_id: company-run-sec-10y-100-company-dashboard
entity_type: company_thesis_dashboard
source: flywheel-ideas
date: '2026-05-08'
created: '2026-05-08T16:38:07.564Z'
---
# Thesis Dashboard sec-10y-100-company

One-page decision-support view: what failed, what lesson survived, what still needs review, and which bets remain live.

## Snapshot

- Filings scanned: 1468
- Companies tracked: 37
- Current open bets: 349
- Accepted failures: 70
- Lessons recorded: 70
- Missing lesson memos: 0
- Review events remaining: 0
- Staged candidates remaining: 31

## Start Here

- Proof path: [[reports/company-runs/sec-10y-100-company/proof-path|Proof path]]
- Full [[thesis]]: [[reports/company-runs/sec-10y-100-company/thesis|Company thesis]]
- Evidence tracker: [[reports/company-runs/sec-10y-100-company/tracker|Evidence tracker]]
- Accepted lessons: [[reports/company-runs/sec-10y-100-company/accepted-lessons|Accepted lessons]]
- Human review queue: [[reports/company-runs/sec-10y-100-company/review-queue|Human review queue]]
- Candidate noise: [[reports/company-runs/sec-10y-100-company/candidate-noise|Candidate noise]]
- Cross-sector patterns: [[reports/company-runs/sec-10y-100-company/cross-sector-patterns|Cross-sector patterns]]

## What Failed

- **KO / Demand**
  - Lesson: Consumer-brand demand risk can become decision-grade through activism and geopolitical boycotts before the filing quantifies the lost revenue.
  - Evidence: 1 accepted failure(s), outcome(s) out-UBieSNC8
  - Context: KO disclosed that it had faced product boycotts resulting from activism, reducing demand for its products.
- **HD / Demand**
  - Lesson: Retail demand pressure becomes decision-grade when negative comparable sales create cost deleverage, not only when headline revenue declines.
  - Evidence: 1 accepted failure(s), outcome(s) out-7F4mfxw6
  - Context: HD reported increased depreciation and amortization, including deleverage from a negative comparable sales environment.
- **MSFT / Liquidity**
  - Lesson: Liquidity and credit assumptions should watch bad debt expense as an early signal of customer payment stress, even inside otherwise growing enterprise businesses.
  - Evidence: 1 accepted failure(s), outcome(s) out-nCkT2qHA
  - Context: MSFT reported higher sales and marketing expense partly due to increased bad debt expense.
- **PM / Geopolitics and tariffs**
  - Lesson: Regional distribution exposure should be tracked as a channel and geopolitical dependency when termination can flow directly through net revenue.
  - Evidence: 1 accepted failure(s), outcome(s) out-5M7w88HP
  - Context: PM recorded an $80 million pre-tax charge, $70 million net of tax, as a reduction of net revenues after terminating a Middle East distribution arrangement.
- **PM / Geopolitics and tariffs**
  - Lesson: When geopolitical conflict cancels market launches, the risk should be tracked as a realized operating and capital-allocation failure, not only as regional uncertainty.
  - Evidence: 1 accepted failure(s), outcome(s) out-ATHaEBdv
  - Context: PM recorded approximately $80 million of second-quarter 2022 charges and $122 million for the first half of 2022 related to the war in Ukraine.
- **WBD / Demand**
  - Lesson: Media demand risk is decision-grade when revenue softness flows through content expense, participant payments, or impairment language rather than only audience commentary.
  - Evidence: 1 accepted failure(s), outcome(s) out-awfUrwLp
  - Context: WBD reported a 41% decrease in theatrical product content expense tied to lower theatrical product revenue and a games-content impairment.
- **VZ / Demand**
  - Lesson: Advertising-supported segments need separate demand tracking because macro shocks can hit advertiser spend and commercial-search intent before broader company demand weakens.
  - Evidence: 1 accepted failure(s), outcome(s) out-zPo6jwP7
  - Context: Verizon Media experienced declines in advertising and search revenue as advertisers paused or canceled campaigns and users searched fewer commercial terms.
- **TMUS / Customer and platform dependency**
  - Lesson: Partner-dependency assumptions should treat OEM quality failures as first-order financial risk when the company sells, finances, or stocks the affected devices.
  - Evidence: 1 accepted failure(s), outcome(s) out-r9mw4Nbw
  - Context: TMUS recorded no revenue for recalled device sales, impaired devices to net realizable value, and booked an OEM reimbursement receivable.

## What Still Needs Review

No staged review events are waiting for human judgment.
31 staged candidate(s) are documented as duplicate or closed-assumption pressure in [[reports/company-runs/sec-10y-100-company/candidate-noise|Candidate noise]].

## Current Bets With Review Pressure

- **AAPL / Liquidity** - pressure 0, 77 observation(s), latest 2026-05-01
  - Assumption: asm-Ci3UyXkx
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **AAPL / AI, R&D, and technology** - pressure 0, 66 observation(s), latest 2026-05-01
  - Assumption: asm-K7yH9uXU
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **AAPL / Macro and foreign exchange** - pressure 0, 65 observation(s), latest 2026-05-01
  - Assumption: asm-yxrE5cms
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **AAPL / Regulation and legal** - pressure 0, 64 observation(s), latest 2026-05-01
  - Assumption: asm-ZbiY6z1v
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **AAPL / Inventory and channel** - pressure 0, 59 observation(s), latest 2026-05-01
  - Assumption: asm-kAXYVk2E
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **VZ / Customer and platform dependency** - pressure 0, 50 observation(s), latest 2026-05-01
  - Assumption: asm-vNY9Aig5
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **VZ / Cloud and data center capacity** - pressure 0, 48 observation(s), latest 2026-05-01
  - Assumption: asm-3bGK86qL
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.
- **AAPL / Cloud and data center capacity** - pressure 0, 47 observation(s), latest 2026-05-01
  - Assumption: asm-Dh2ntKH6
  - Why it matters: Recurring issuer disclosures make this a live dependency to keep tracking.

## Cross-Company Mechanisms

- **Regulation and legal**: 32 open bet(s), 2053 observation(s), pressure 0, companies AAPL, ABNB, AMZN, AVGO, BKNG, CMCSA, COST, EA, [[googl]], HD, INTC, KO, LOW, MAR, MCD, MO, MSFT, MU, NFLX, [[nvda]], ORCL, PEP, PG, PLTR, PM, SBUX, T, TJX, TMUS, TSLA, VZ, WMT
- **Liquidity**: 29 open bet(s), 2008 observation(s), pressure 0, companies AAPL, ABNB, AMD, AMZN, AVGO, CMCSA, COST, CSCO, [[dis]], EA, GOOGL, HD, INTC, KO, MAR, META, MU, NFLX, NVDA, ORCL, PEP, PG, PLTR, SBUX, T, TMUS, TSLA, WBD, WMT
- **Macro and foreign exchange**: 31 open bet(s), 1784 observation(s), pressure 0, companies AAPL, ABNB, AMZN, BKNG, CMCSA, COST, CSCO, EA, GOOGL, HD, INTC, KO, LOW, MAR, MCD, MO, MSFT, MU, NFLX, NVDA, ORCL, PEP, PG, PLTR, PM, SBUX, T, TJX, TMUS, WBD, WMT
- **Competition**: 36 open bet(s), 1640 observation(s), pressure 0, companies AAPL, ABNB, AMD, AMZN, AVGO, BKNG, CMCSA, COST, DIS, EA, GOOGL, HD, INTC, KO, LOW, MAR, MCD, META, MO, MSFT, MU, NFLX, NVDA, ORCL, PEP, PG, PLTR, PM, SBUX, T, TJX, TMUS, TSLA, VZ, WBD, WMT
- **Demand**: 21 open bet(s), 1502 observation(s), pressure 0, companies ABNB, AMZN, AVGO, CMCSA, COST, EA, GOOGL, INTC, LOW, MAR, MCD, MO, MSFT, NFLX, ORCL, PEP, PG, PLTR, T, TJX, WMT
- **Cloud and data center capacity**: 35 open bet(s), 1291 observation(s), pressure 0, companies AAPL, ABNB, AMZN, AVGO, BKNG, CMCSA, COST, CSCO, DIS, EA, GOOGL, HD, INTC, KO, LOW, MAR, MCD, META, MO, MSFT, MU, NFLX, NVDA, ORCL, PEP, PLTR, PM, SBUX, T, TJX, TMUS, TSLA, VZ, WBD, WMT
- **AI, R&D, and technology**: 34 open bet(s), 1274 observation(s), pressure 0, companies AAPL, ABNB, AMD, AMZN, AVGO, BKNG, CMCSA, COST, CSCO, DIS, EA, GOOGL, HD, INTC, KO, LOW, MAR, MCD, META, MO, NFLX, NVDA, ORCL, PEP, PG, PLTR, SBUX, T, TJX, TMUS, TSLA, VZ, WBD, WMT
- **Inventory and channel**: 31 open bet(s), 1219 observation(s), pressure 0, companies AAPL, ABNB, AMZN, AVGO, BKNG, CMCSA, COST, CSCO, DIS, EA, GOOGL, HD, INTC, KO, LOW, MAR, MCD, META, MO, MSFT, NFLX, ORCL, PEP, PG, T, TJX, TMUS, TSLA, VZ, WBD, WMT

## Product Value Check

- Novel value today: persistent assumption memory, explicit human outcome adjudication, refuted assumptions, lesson memos, and an auditable Markdown trail.
- Not novel by itself: SEC data, filing search, risk-factor extraction, or generic summarization.
- Practical use: analyst memory. It shows which thesis assumptions have failed before and what similar live bets still deserve review.

## Flywheel Audit

```flywheel-audit-json
{
  "schema": "sec-company-ledger-markdown-v1",
  "run_id": "sec-10y-100-company",
  "entity_id": "company-run-sec-10y-100-company-dashboard",
  "entity_type": "company_thesis_dashboard",
  "path": "reports/company-runs/sec-10y-100-company/dashboard.md"
}
```
