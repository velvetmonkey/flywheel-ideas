...
describe('company tracker', () => {
  ...
  it('runs the full SEC product loop across 5 sectors with 3 companies each', async () => {
    ...
    expect(proofPath).toContain(`[[reports/company-runs/${result.run_id.toLowerCase()}/cross-sector-patterns|Cross-sector patterns]]`);
    expect(proofPath).toContain('Recurring risk disclosure');
    const crossSectorPatterns = await fsp.readFile(path.join(vault, `reports/company-runs/${result.run_id.toLowerCase()}/cross-sector-patterns.md`), 'utf8');
    expect(crossSectorPatterns).toContain('Accepted Lessons');
    expect(crossSectorPatterns).toContain('Open Pressure');
    const patternDir = path.join(vault, `reports/company-runs/${result.run_id.toLowerCase()}/patterns`);
    const [patternFile] = await fsp.readdir(patternDir);
    expect(patternFile).toBeDefined();
    const patternPath = path.join(patternDir, patternFile);
    const patternPage = await fsp.readFile(patternPath, 'utf8');
    expect(patternPage).toContain('## Accepted Failure Chains');
    expect(patternPage).toContain('## Live Bets With Pressure');
    expect(patternPage).toContain('## Pending Review Evidence');
